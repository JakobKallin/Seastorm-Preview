...
seastorm.monitor = function(vessels, userFiles, onProgress, fileCache) {
	...
	*/
	function uploadFile(filename, contents, vessel) { 
		fileCache[vessel.name] = fileCache[vessel.name] || {};
		
		if ( fileCache[vessel.name][filename] === contents ) {
			return Promise.resolve();
		}
		else {
			fileCache[vessel.name][filename] = contents;
			return seattle.call(
				vessel.ip,
				vessel.port,
				'AddFileToVessel',
	...
	function readStatus(vessel) {
		return seattle.call(vessel.ip, vessel.port, 'GetVessels', [])
		.then(function(string) {
			var statuses = seattle.parseVesselStatus(string);
