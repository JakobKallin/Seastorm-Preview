...
seastorm.ProgramController = function(
	...
	$scope.reset = function() {
		fileCache = {};
		var vessels = $scope.model.programs.filter(get('isActive')).map(get('vessel'));
		return Promise.all(vessels.map(resetVessel)).catch(function(error) {
			$scope.$apply(function() {
				handleError(error);
			});
		});
	...
	function savePrograms(programs) {
		...
		programs.forEach(function(program) {
			...
			programTable[seattle.vesselId(program.vessel)] = copy;
		...
		localStorage.programs = angular.toJson(programTable);
	}
};